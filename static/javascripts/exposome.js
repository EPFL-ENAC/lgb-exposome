$(document).ready(function () {
  ...
  $("#search_NPA").on("blur", function () {
    ...
    $.getJSON("https://api3.geo.admin.ch/rest/services/api/MapServer/find?layer=ch.swisstopo-vd.ortschaftenverzeichnis_plz&searchText=" + $("#search_NPA").val() + "&searchField=plz", function (el) {
...
function init() {
  ...
  map.on('click', function (e) {
    ...
    let clickedCoordinate = ol.proj.transform(e.coordinate, 'EPSG:3857', 'EPSG:4326');
    // Appel à la fonction qui va récupérer les données d'exposome via l'API Django
    fetchAndDisplayExposomeData(clickedCoordinate[0], clickedCoordinate[1], e.coordinate);
  });
}
...
async function handleSearchByAddress() {
  ...
  document.getElementById("npa_ville").textContent = searchNPA + "  " + searchVille;

  try {
    // Appel à l'API Django pour trouver les coordonnées
    const response = await fetch(`/api/find_coordinates/?address=${encodeURIComponent(searchAdress)}&npa=${encodeURIComponent(searchNPA)}&city=${encodeURIComponent(searchVille)}`);
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(errorData.error || 'Erreur lors de la recherche des coordonnées.');
    }
...
function handleSearchByMapClick() {
  ...
  draw.on('drawend', function (evt) {
    ...
    const coordinate_map_lonlat = ol.proj.transform(coordinate_map_proj, 'EPSG:3857', 'EPSG:4326');
    
    map.removeInteraction(draw); // Retire l'interaction de dessin après le clic

    // Appel à la fonction pour récupérer et afficher les données d'exposome
    fetchAndDisplayExposomeData(coordinate_map_lonlat[0], coordinate_map_lonlat[1], coordinate_map_proj);
  });
}

// Fonction principale pour récupérer et afficher les données d'exposome
async function fetchAndDisplayExposomeData(longitude, latitude, olCoordinateForMap) {
  try {
    // Appel à l'API Django pour obtenir les données d'exposome
    const response = await fetch(`/api/get_exposome_data/?longitude=${longitude}&latitude=${latitude}`);
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(errorData.error || 'Erreur lors de la récupération des données d\'exposome.');
    }
    const data = await response.json();
    ...
    document.getElementById("ndvi_value").textContent = `${data.hectare_data.RELI_ndvi} (Canton : ${data.canton_stats.avg_ndvi.toPrecision(3)}, Suisse : ${data.suisse_stats.ndvi})`;
    document.getElementById("income_value").textContent = `${data.hectare_data.RELI_income} (Canton : ${data.canton_stats.median_income.toPrecision(5)}, Suisse : ${data.suisse_stats.income})`;
    document.getElementById("noise_value").textContent = `${data.hectare_data.RELI_noise} (Canton : ${data.canton_stats.median_noise.toPrecision(2)}, Suisse : ${data.suisse_stats.noise})`;
    document.getElementById("pm25_value").textContent = `${data.hectare_data.RELI_pm25} (Canton : ${data.canton_stats.median_pm25}, Suisse : ${data.suisse_stats.pm25})`;
    
    // Met à jour l'overlay pour les informations de l'hectare cliqué
    const overlayContainerElement = document.querySelector('.overlay-container');
    const overlayLayer = map.getOverlays().getArray().find(overlay => overlay.getElement() === overlayContainerElement);
    if (overlayLayer) {
        overlayLayer.setPosition(olCoordinateForMap);
        document.getElementById('feature-RELI').innerHTML = `Canton: ${data.canton}`;
        document.getElementById('feature-NDVI').innerHTML = `NDVI: ${data.hectare_data.RELI_ndvi}`;
        document.getElementById('feature-Income').innerHTML = `Revenu moyen: ${data.hectare_data.RELI_income}`;
        document.getElementById('feature-PM25').innerHTML = `PM 2.5: ${data.hectare_data.RELI_pm25}`;
        document.getElementById('feature-Noise').innerHTML = `Bruit routier: ${data.hectare_data.RELI_noise}`;
    }
    ...
    console.error("Erreur lors de la récupération des données d'exposome:", error);
...
function reinitialise() {
  ...
  for (let i = 3; i < layers.length; i++) { // Toutes les couches d'hectares
...
function addHectareLayers(HectareStyle) {
  const cantons = [
    'vaud', 'geneve', 'jura', 'neuchatel', 'fribourg', 'bern', 'basel', 'solothurn',
    'valais', 'graubunden', 'ticino', 'uri', 'schwyz', 'glarus', 'aargau', 'luzern',
    'nidwalden', 'obwalden', 'zug', 'zurich', 'schaffhausen', 'thurgau', 'st_gallen',
    'appenzell_ausser', 'appenzell_inner'
  ];
